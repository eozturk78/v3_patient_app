...
class _BluetoothExamplePageState extends State<BluetoothExamplePage> {
  ...
  static const String BLOOD_PRESSURE_SERVICE_UUID = "1810";
  static const String BLOOD_PRESSURE_MEASUREMENT_UUID = "2a35";
  ...
  void connectToDevice(DiscoveredDevice device) {
    _connectionSubscription = flutterReactiveBle.connectToDevice(
      ...
      servicesWithCharacteristicsToDiscover: {
        Uuid.parse(BLOOD_PRESSURE_SERVICE_UUID): [Uuid.parse(BLOOD_PRESSURE_MEASUREMENT_UUID)]
      },
  ...
  void _handleConnectionState(DiscoveredDevice device, ConnectionStateUpdate connectionState) {
    ...
    for (var service in services) {
      if (service.serviceId == Uuid.parse(BLOOD_PRESSURE_SERVICE_UUID)) {
        for (var characteristic in service.characteristics) {
          if (characteristic.characteristicId == Uuid.parse(BLOOD_PRESSURE_MEASUREMENT_UUID)) {
  ...
  void subscribeToCharacteristic(String deviceId, DiscoveredCharacteristic characteristic) {
    final characteristicSubscription = flutterReactiveBle.subscribeToCharacteristic(
  ...
  String _parseBloodPressureData(List<int> data) {
    ...
    int systolic = (data[1] << 8) | data[2];
    int diastolic = (data[3] << 8) | data[4];
    int pulse = (data[14] << 8) | data[15];

    int year = (data[7] << 8) | data[8];
    int month = data[9];
    int day = data[10];
    int hour = data[11];
    int minute = data[12];
